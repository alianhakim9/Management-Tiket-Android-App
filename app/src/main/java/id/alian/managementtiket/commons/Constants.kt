package id.alian.managementtiket.commons

object Constants {
    const val BASE_URL = "https://88ed-103-147-9-54.ngrok.io/api/"
    const val PREFERENCES_NAME = "ticket_preferences"
    const val TOKEN_KEY = "BearerToken"
    const val USER_NAME_KEY = "UserName"

    const val ERROR_MESSAGE = "Couldn't reach server. Check your internet connection"
    const val UNEXPECTED_ERROR_MESSAGE = "an unexpected error occurred"

    const val TICKET_STARTING_PAGE_INDEX = 1
    const val NETWORK_PAGE_SIZE = 10
}